...
spec <- create_spec(
  ...
  ingest = list(
    # Point this to the folder holding the raw accelerometer files.
    # It can be the workspace data_raw folder, or it can be an external path.
    raw_dir = "data_raw"
  ),
  labels = list(
...
If your raw files already live somewhere else, you can replace
`"data_raw"` with an absolute path such as
`"E:/portable_drive/cattle_trial_raw"`. `moover` will read from that
folder and still keep the epoch dataset, model bundle, and test vectors
inside the workspace for this run.
...
If the raw files are large, `moover` can also read them in fixed-size
chunks using `ingest$chunk_rows`. That option is there to make large
local workflows more manageable, not to change the feature definitions
or the final model outputs.
