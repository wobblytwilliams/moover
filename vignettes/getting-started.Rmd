...
If you already have a model and only want predictions, jump to [Chapter 6: Predict Behaviour with a Shipped Model](train-and-predict-with-real-data.html#walkthrough-2).
...
The bigger idea behind `moover` is that the workflow should feel ordinary. If you can load a package, read in a table, and make a plot in R, you should be able to train a behaviour model and use it for prediction. You should not need to become a precision-livestock specialist first.

We'll do four things in this chapter:

1. find the example training workspace that ships with `moover`
...
Once you switch to your own project, the core requirement is still simple: give `moover` accelerometer data in a standard format, a `tech.csv` file that links animals and sensors, and an `observations.csv` file with labelled behaviour periods. From there, `moover` handles the feature generation, Random Forest training, validation, export, and prediction steps.

## Find and copy the example training workspace

The package ships with a curated real-data training workspace. We don't run directly inside the installed package folder, because installed package files should be treated as read-only. Instead, we copy that example workspace into a folder we can write to.

```{r, eval = FALSE}
library(moover)

# Find the packaged real-data training workspace that ships inside moover.
# This folder already contains curated raw accelerometer files, a tech table,
# and a labelled observations table.
example_dir <- system.file("extdata", "example_train_workspace", package = "moover")
...
- we point `moover` to the copied training workspace
